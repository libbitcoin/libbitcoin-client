...
build_all()
{
    ...
    if [[ ! ($CI == true) ]]; then
        create_from_github evoskuil libbitcoin-client master
        build_from_github_cmake libbitcoin-client "$PARALLEL" true "${BITCOIN_CLIENT_OPTIONS[@]}" $CUMULATIVE_FILTERED_ARGS_CMAKE "$@"
    else
