...
build_all()
{
    ...
    build_from_github libbitcoin-system libbitcoin-protocol version3 "$PARALLEL" "${BITCOIN_PROTOCOL_OPTIONS[@]}" "$@"
    build_from_travis libbitcoin-system libbitcoin-client version3 "$PARALLEL" "${BITCOIN_CLIENT_OPTIONS[@]}" "$@"
}
