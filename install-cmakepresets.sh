...
handle_custom_options()
{
    ...
    REPO_PRESET[libbitcoin-client]="$PRESET_ID"
    display_message "REPO_PRESET[libbitcoin-client]=${REPO_PRESET[libbitcoin-client]}"
...
build_all()
{
    ...
    if [[ ! ($CI == true) ]]; then
        ...
        build_from_github_cmake libbitcoin-client ${REPO_PRESET[libbitcoin-client]} "$PARALLEL" true                 "yes" "${BITCOIN_CLIENT_OPTIONS[@]}" $CUMULATIVE_FILTERED_ARGS_CMAKE "$@"
    else
